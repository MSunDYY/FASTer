        ...
        for key, val in data_dict.items():
            try:
                if key in ['voxels', 'voxel_num_points']:
                    ...
                    ret[key] = torch.concat(val, 0)
                elif key in ['trajectory_rois']:
                    max_num_roi = max([x.shape[1] for x in val])
                    ret[key] = torch.concat(val, 1)
                    ...
                    if val[0]['stack_concat']:
                        for sub_key in val[0].keys():
                            ...
                            temp[sub_key] = torch.concat(sub_val, dim=1) if sub_key in ['trajectory_rois',
                                                                                        'valid_length'] else torch.concat(
                                sub_val, dim=0)
                    else:
                        for sub_key in val[0].keys():
                            if sub_key == 'stack_concat':
