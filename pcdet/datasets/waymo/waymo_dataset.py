...
class WaymoDataset(DatasetTemplate):
    ...
    def get_sequence_data(self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False, load_prev_gt_boxes=False):
        ...
        for idx, sample_idx_pre in enumerate(sample_idx_pre_list):
            ...
            if sequence_cfg.get('ONLINE_DETECT',None) is not None:
                if not self.training:
                    num_points_pre.append(0)
                    continue
                else:
                    lidar_file = self.root_path / sequence_cfg.ONLINE_DETECT.POINTS_PRE_TAG / sequence_name / ('%04d.npy' % sample_idx_pre)
                    points_pre = np.load(lidar_file)
            else:
                points_pre = self.get_lidar(sequence_name, sample_idx_pre)
            pose_pre2cur = np.matmul(np.linalg.inv(pose_cur),pose_pre)
            points_pre[:,:3] = np.matmul(points_pre[:,:3],pose_pre2cur[:3,:3].T)
            points_pre[:,:3] += pose_pre2cur[None,:3,-1]
        ...
        if load_pred_boxes:
            ...
            temp_pred_boxes = temp_pred_boxes
            pred_boxes = temp_pred_boxes[:,:, :9]
            pred_scores = temp_pred_boxes[:,:, 9]
            pred_labels = temp_pred_boxes[:,:, 10].astype(np.int64)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg_file', type=str, default='cfgs/waymo_models/faster_16frames.yaml', help='specify the config of dataset')
    parser.add_argument('--func', type=str, default='create_waymo_rois_points', help='')
