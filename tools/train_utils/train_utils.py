...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                ...
                use_logger_to_record=False, logger=None, logger_iter_interval=None, ckpt_save_time_interval=None, show_gpu_stat=False, cfg=None,eval_loader = None):
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if (type(model).__name__) == 'FASTer' and cur_epoch in [3,5]:
                ...
                logger.info('Start generating points for training!') # avoid heavy loads on CPU and accelerate training
                for i,batch_dict in tqdm.tqdm(enumerate(eval_loader),dynamic_ncols=True,leave=True,ascii=True):
                    load_data_to_gpu(batch_dict)
                    with torch.no_grad():
                        model.simple_refer(batch_dict,cur_epoch)
